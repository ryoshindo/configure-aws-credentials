...
describe("Configure AWS Credentials", () => {
  ...
  test("validates region name", async () => {
    ...
    expect(core.setFailed).toHaveBeenCalledWith(
      "Region is not valid: $AWS_REGION"
    );
  });
  ...
  test("basic role assumption exports", async () => {
    ...
    expect(core.setSecret).toHaveBeenNthCalledWith(
      5,
      FAKE_STS_SECRET_ACCESS_KEY
    );
    ...
    expect(core.exportVariable).toHaveBeenNthCalledWith(
      7,
      "AWS_SESSION_TOKEN",
      FAKE_STS_SESSION_TOKEN
    );

    expect(core.setOutput).toHaveBeenNthCalledWith(
      2,
      "aws-account-id",
      FAKE_ROLE_ACCOUNT_ID
    );
  });
  ...
  test("assume role can pull source credentials from self-hosted environment", async () => {
    core.getInput = jest
      .fn()
      .mockImplementation(
        mockGetInput({ "role-to-assume": ROLE_ARN, "aws-region": FAKE_REGION })
      );
  ...
  test("role assumption tags", async () => {
    ...
    expect(mockStsAssumeRole).toHaveBeenCalledWith({
      ...
      Tags: [
        ...
        {
          Key: "Repository",
          Value: ENVIRONMENT_VARIABLE_OVERRIDES.GITHUB_REPOSITORY,
        },
        {
          Key: "Workflow",
          Value: ENVIRONMENT_VARIABLE_OVERRIDES.GITHUB_WORKFLOW,
        },
  ...
  test("role assumption duration provided", async () => {
    core.getInput = jest
      .fn()
      .mockImplementation(
        mockGetInput({ ...ASSUME_ROLE_INPUTS, "role-duration-seconds": 5 })
      );
  ...
  test("role external ID provided", async () => {
    core.getInput = jest
      .fn()
      .mockImplementation(
        mockGetInput({ ...ASSUME_ROLE_INPUTS, "role-external-id": "abcdef" })
      );
  ...
  test("masks variables before exporting", async () => {
    ...
    core.exportVariable.mockImplementation((name, value) => {
      if (!maskedValues.includes(value) && !publicFields.includes(name)) {
        throw new Error(
          value + " for variable " + name + " is not masked yet!"
        );
      }
    });
